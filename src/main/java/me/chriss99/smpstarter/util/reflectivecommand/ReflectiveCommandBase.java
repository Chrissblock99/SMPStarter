...
public abstract class ReflectiveCommandBase extends BukkitCommand implements CommandExecutor, TabCompleter {
    ...
    public ReflectiveCommandBase(@NotNull String commandName, @NotNull String description, @NotNull List<String> aliases, @NotNull CustomClassParser classParser, @NotNull ParameterTabCompleter tabCompleter) {
        ...
        PluginCommand pluginCommand = SMPStarter.getInstance().getCommand(commandName);
    ...
    public boolean execute(@NotNull CommandSender sender, @NotNull String alias, @NotNull String[] arguments) {
        ...
        if (Player.class.equals(parsedSubCommand.subCommand().getSenderParameterType()) && !(sender instanceof Player)) {
            sender.sendMessage(SMPStarter.getMiniMessage().deserialize("<red>Only players can use this subCommand!"));
            return true;
        }
        ...
        try {
            ...
            sender.sendMessage(SMPStarter.getMiniMessage().deserialize("<red>The programmer of this subCommand used the ReflectiveCommandBase incorrectly!\n<dark_red>IllegalAccessException: <red>" + iae.getMessage()));
            iae.printStackTrace();
        } catch (InvocationTargetException e) {
            Throwable cause = e.getCause();
            sender.sendMessage(SMPStarter.getMiniMessage().deserialize("<red>The subCommand method \"" + parsedSubCommand.subCommand().getMethod().getName() + "\" threw an exception!\n<dark_red>" +
    ...
    private @NotNull Component noPathMatchingSubCommandsMessage(final @NotNull ArrayList<String> possiblePaths) {
        ...
        return SMPStarter.getMiniMessage().deserialize(stringBuilder.toString());
    }
